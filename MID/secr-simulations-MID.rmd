...
What percentage of individuals were ghosts? 
```{r}
fn <- function(ch) {
    c(n = nrow(ch), 
     nghost = sum(grepl('N',rownames(ch))),
     pctghost = sum(grepl('N',rownames(ch))) / nrow(ch) * 100)
}
MIDsims2pg <- run.scenarios(
    nrepl       = 1000, 
    scenarios   = scen2, 
    trapset     = grid, 
    det.args    = detarg2, 
    ...
    CH.function = "simMID.capthist")
rbind(
sapply(MIDsims2pg$output, apply, 2, mean),
t(attr(detarg2, "comb"))
)
```
