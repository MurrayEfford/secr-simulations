...
nrepl   <- 500
nrepl2  <- 200
metadata(runsim, extra = runextra) # if not run picks up saved metadata.RDS
```
...
### Repeat with elongated grid

```{r ARR2 scenarios, eval = TRUE, message = FALSE, results = "hide"}
grid2 <- make.grid(nx = 16, ny = 4, spacing = 40, detector = "proximity")
fitarg <- expand.arg(detectfn = c(14,16))
ratio <- seq(0.5,2.5,0.25)
R95 <- max(dist(grid2)) / ratio
scen2 <- make.scenarios(D = 12.5, noccasions = 10, detectfn = 14, 
    lambda0 = 0.1, sigma = R95/2/2.45, fitindex = 1:2)
# scale density to avoid variation in # detections due to sigma
# result corresponds to D = 12.5 when sigma = 20 (outside simulated range)
scen2$D <- scen2$D / (scen2$sigma / 20)^2
```

```{r ARR2 run, eval = runextra, cache = cache, warning = FALSE, message = FALSE}
ARR2sims <- run.scenarios(
    nrepl     = nrepl2, 
    scenarios = scen2, 
    trapset   = grid2, 
    nx        = 128,         # increase to maintain approximately same spacing
    fit       = TRUE, 
    fit.args  = fitarg, 
    seed      = 12345)
saveRDS(ARR2sims, file = 'ARR2sims.RDS')
```
...
## Supplementary results 2025-03-28

```{r ARR2 results, eval = TRUE, echo = FALSE}
ARR2sims <- readRDS(file = 'ARR2sims.RDS')
```

ARR2sims completed with `r nrepl2` replicates on `r nc` threads in `r round(ARR2sims$proctime/60,1)` minutes.

```{r ARR2 results2, eval = TRUE, echo = TRUE}
estD <- estimateSummary(ARR2sims)
estD
```

```{r ARR2 figure, eval = figures, echo = FALSE, fig.width = 8, fig.height = 4, fig.cap = "Effect of array size on relative bias (RB) and relative root-mean-square error (rRMSE) of density estimates. Elongated grid."}

par(mfrow=c(1,2), mar=c(4,4,1,1), mgp=c(2.4,0.7,0), bty = 'o', cex = 1, pty='s')

x <- max(dist(grid2)) / (2.45 * scen2$sigma[1:9] *2)
