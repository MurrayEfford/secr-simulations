...
```{r CLO code1, eval = FALSE}
CLOcounts <- run.scenarios(nrepl = nrepl, scen, trapset = grid, pop.args = poparg, 
                           det.args = detarg, joinsessions = TRUE, seed = 12345)

CLOsims <- run.scenarios(nrepl = nrepl, scen, trapset = list(grid), pop.args = poparg, 
                         det.args = detarg, joinsessions = TRUE, fit = TRUE, 
                         fit.args = fitarg, seed = 12345)

saveRDS(CLOcounts, file = 'CLOcounts.RDS')
...
CLOcounts <- readRDS('CLOcounts.RDS')
CLOsims <- readRDS('CLOsims.RDS')
# header(CLOsims)
outraw  <- summary(CLOcounts)$OUTPUT
...
n <- sapply(outraw, '[', 'n','mean')
r <- sapply(outraw, '[', 'r','mean')
m <- sapply(outraw, '[', 'nmov','mean')
```

Runtime for `r CLOsims$nrepl` replicates using `r nc` cores was `r round(CLOsims$proctime/60)` minutes.

```{r CLO tables1, echo = FALSE}
data.frame(Days = nocc*7, Survival = phiw^52, n=n, r=r, m=m, RB=RB, RSE=RSE, COV = COV)
```
...
phi1 <- 7:12
phi2 <- 13:18
model0 <- lm(I(RB[phi0]) ~ I(nocc*7))
model1 <- lm(I(RB[phi1]) ~ I(nocc*7))
model2 <- lm(I(RB[phi2]) ~ I(nocc*7))
...
```{r CLO figures1, echo = FALSE, fig.height = 3, fig.width=7, out.width="100%", fig.cap = "Relative bias RB, relative standard error RSE, and coverage of 95% confidence intervals COV for density estimates from studies of varying duration. Annual survival 50% (open circles), 75% (filled circles) or 100% (triangles). Horizontal lines for reference only."}
...
abline(model2)
points(nocc*7, RB[phi0])
points(nocc*7, RB[phi1], pch = 16)
points(nocc*7, RB[phi2], pch = 2)
abline(h=0, lty=2, col='grey')

plot(0,0,type='n',ylim=c(0,0.3), xlim=c(0,365/2), xlab = 'Duration (days)', ylab='RSE', xaxs = 'i')
points (nocc*7, RSE[phi0])
points (nocc*7, RSE[phi1], pch=16)
points(nocc*7, RSE[phi2], pch = 2)
abline(h=0.1, col='grey')

plot(0,0,type='n',ylim=c(0,1), xlim=c(0,365/2), xlab = 'Duration (days)', ylab='COV', xaxs = 'i')
points(nocc*7, COV[phi0])
points(nocc*7, COV[phi1], pch = 16)
points(nocc*7, COV[phi2], pch = 2)
