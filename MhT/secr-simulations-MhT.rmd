...
Note: could compare to PLB $\hat \lambda$ by setting renumber = FALSE in sim.popn.
...
fitargo <- list(detectfn = 'HHN', type='PLBsecrl', model = lambda~1)
...
```{r MhT table, eval = TRUE, echo = FALSE, results = "hold"}
options(width=110, digits=4)
MhTsims <- readRDS(file = 'MhTsims.RDS')
onesim <- function (sim, parm = 'D1', fn = mean) {
    fn(sapply(sim, '[', parm,'estimate'))
}
...
df <- data.frame(
    ...
    RBD       = sapply(MhTsims$output, onesim, 'D1', fn=mean) / trueD - 1,
    seRBD     = sapply(MhTsims$output, FUN = onesim, parm='D1', fn=se) / trueD,
    lambda    = sapply(MhTsims$output, onesim, 'lambda1'),
    selambda  = sapply(MhTsims$output, FUN = onesim, parm='lambda1', fn=se) 
)
df
```
```{r PLB table, eval = TRUE, echo = FALSE, results = "hold"}
PLBsims <- readRDS(file = 'PLBsims.RDS')
dfPLB <- data.frame(
    CVlambda0 = rep(seq(0,0.8,0.2),2),
    truelambda = rep(c(0.9,1.1), each = 5),
    lambda = sapply(PLBsims$output, onesim, 'lambda'),
    selambda = sapply(PLBsims$output, FUN = onesim, parm='lambda', fn=se) 
)
dfPLB
```
 
```{r MhT figure, eval = figures, echo = FALSE, fig.width = 8, fig.height = 4, fig.cap = "Relative bias of parameter estimates from null model given individual variation" }
MhTsims <- readRDS(file = 'MhTsims.RDS')
...
plot(0,0,type='n', xlim=c(0,0.8), ylim=c(-0.4,0.4), xlab = cvltext, ylab='RB')
abline(h=0, lty=2)
...
```
```{r PLB figure, eval = figures, echo = FALSE, fig.width = 8, fig.height = 4, fig.cap = "Relative bias of parameter estimates from null model given individual variation" }
PLBsims <- readRDS(file = 'PLBsims.RDS')
cvltext <- expression(paste("CV( ", lambda[0], " )"))
x <- seq(0,0.8,0.20)
offset <- 0.005
